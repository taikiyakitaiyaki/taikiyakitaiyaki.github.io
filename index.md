<div style="display: flex; align-items: flex-start; gap: 20px;">

  <img src="assets/profil.jpg"
       alt="Profilbilde"
       style="width:150px; border-radius:50%;" />

  <div>

## Hei, jeg heter Trinh My Hoang

Jeg er student i informasjonssystemer ved Universitetet i Agder og søker praksisplass høsten 2026.
Jeg er interessert i hvordan IT‑systemer brukes i praksis i organisasjoner, og ønsker å utvikle både tekniske og analytiske ferdigheter gjennom praksis.

Jeg jobber strukturert, liker å lære nye ting og trives godt med samarbeid i gruppe.

---

## Kompetanse

**Programmering**
- Grunnleggende Python
- Java
- SQL

**Web og systemer**
- HTML og CSS
- Forståelse for systemanalyse og kravspesifikasjon

**Verktøy**
- Git og GitHub
- Visual Studio Code
- Dokumentasjon og rapportering

---

## Prosjekter og erfaring

**Skoleprosjekter ved UiA**
- Arbeid med programmeringsoppgaver og gruppeprosjekter
- Fokus på analyse, problemløsning og samarbeid
- Erfaring med å planlegge og dokumentere arbeid

(GitHub‑prosjekter legges fortløpende til)

---

## Motivasjon for praksis

Jeg ønsker praksis for å få innsikt i:
- hvordan informasjonssystemer brukes i en reell arbeidshverdag
- samarbeid mellom IT og resten av organisasjonen
- praktisk erfaring som utfyller det teoretiske grunnlaget fra studiet

Jeg er motivert for å bidra, lære og utvikle meg faglig gjennom praksisperioden.

---

## Lenker

- GitHub: https://github.com/taikiyakitaiyaki
- LinkedIn: (legg til når klar)
- Video: (valgfritt – kan legges til senere)

---

📧 E‑post: trinhmh@uia.no
``
